...
var parser = require('./riff/parser.js');
require('./chunks/parsers/generators.js');
...
class SoundFont3 {
  ...
  constructor(chunk$1) {
    ...
    if (!(chunk$1 instanceof chunk.SF2Chunk)) {
      const parsedBuffer = parser.parseBuffer(chunk$1);
      chunk$1 = new chunk.SF2Chunk(parsedBuffer);
    }
    if (chunk$1.subChunks.length !== 3) {
      throw new parseError.ParseError("Invalid sfbk structure", "3 chunks", `${chunk$1.subChunks.length} chunks`);
    }
    this.chunk = chunk$1;
    this.metaData = chunk$1.subChunks[0].getMetaData();
    this.sampleData = chunk$1.subChunks[1].getSampleData();
    this.presetData = chunk$1.subChunks[2].getPresetData();
  ...
  getPresets() {
    ...
    const presets = zones.getItemsInZone(
      presetHeaders,
      presetZones,
      presetModulators,
      presetGenerators,
      this.instruments,
      generator.GeneratorType.Instrument
    );
  ...
  getInstruments() {
    ...
    const instruments = zones.getItemsInZone(
      instrumentHeaders,
      instrumentZones,
      instrumentModulators,
      instrumentGenerators,
      this.samples,
      generator.GeneratorType.SampleId
    );
