...
var chunkIterator = require('./chunkIterator.js');
...
class RIFFChunk {
  ...
  getString(position = 0, length) {
    return utils.getStringFromBuffer(this.getBuffer(position, length || this.length - position));
  }
  /**
   ...
   */
  iterator(start = 0) {
    return new chunkIterator.ChunkIterator(this, start);
  }
  /**
  ...
  iterate(callback, start = 0) {
    const iterator = new chunkIterator.ChunkIterator(this, start);
